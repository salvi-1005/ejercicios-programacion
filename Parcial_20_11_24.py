...
#%%4)
merged = pd.merge(
    maiz_produccion_df[['anio', 'rendimiento_kgxha']],
    precipitaciones_anuales_df[['anio', 'promedio_de_acumulado_mensual']],
    on='anio',
    how='inner')

X = merged['rendimiento_kgxha']
Y = merged['promedio_de_acumulado_mensual']

plt.scatter(X, Y)
plt.xlabel('Rendimiento (kg/ha)')
plt.ylabel('Precipitación promedio acumulada (mm)')
plt.show()